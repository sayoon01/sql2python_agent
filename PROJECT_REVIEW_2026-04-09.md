...
현재 프로젝트는 비교 실패 처리와 점수기 기본 보정을 상당 부분 정리했고, 구조도 "MS SQL → Python 코드 변환 agent" 방향으로 단순화됐습니다. 남은 핵심 과제는 모델 메타데이터 중복, 휴리스틱 점수 한계, 테스트 부재입니다.
...
- 실제 DB 연결 기능과 `target_db` 분기가 제거되어 구조가 단순해졌습니다.
- 현재 few-shot과 프롬프트는 MSSQL 실행 코드 생성에 집중되어 있습니다.
...
### 6. DB 연결 및 `target_db` 제거 완료

- 위치: [config/settings.py](/home/keti_spark1/yune/sql2python_v2/sql2python/config/settings.py), [backend/fewshot/builder.py](/home/keti_spark1/yune/sql2python_v2/sql2python/backend/fewshot/builder.py), [frontend/static/js/app.js](/home/keti_spark1/yune/sql2python_v2/sql2python/frontend/static/js/app.js)
- 실제 DB 연결 설정과 `backend/db/connection.py`가 제거됐습니다.
- `target_db` 요청 필드와 프런트 DB 선택 UI도 제거됐습니다.
- 프로젝트는 이제 MSSQL 실행 코드 생성에 집중하는 구조입니다.
...
- 위치: [backend/services/converter/scorer.py](/home/keti_spark1/yune/sql2python_v2/sql2python/backend/services/converter/scorer.py)
- 문법 검사, T-SQL 잔재 감점, 비현실적 DB 패턴 감점이 추가됐지만 여전히 점수의 대부분은 휴리스틱입니다.
...
이 프로젝트는 현재 기준으로 "MS SQL을 실행 가능한 Python 코드로 변환하는 agent" 방향으로 더 선명해졌습니다. DB 연결과 다중 DB 분기 제거로 구조는 단순해졌고, 지금 남은 과제는 생성 품질 검증력과 유지보수성 보강입니다.
